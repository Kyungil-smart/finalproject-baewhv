...
public class RewardButtonUI : RewardIconUI
{
    ...
    {
        if (isContain)
            SetResource(data.CLEAR_REWARD_ICON,
                data.CLEAR_REWARD_NAME,
                data.CLEAR_REWARD_TEXT_ID_02,
                data.CLEAR_REWARD_S_01,
                data.CLEAR_REWARD_S_02,
                data.CLEAR_REWARD_S_03);
        else
            SetResource(data.CLEAR_REWARD_ICON,
                data.CLEAR_REWARD_NAME,
                data.CLEAR_REWARD_TEXT_ID_01,
                data.CLEAR_REWARD_F_01,
                data.CLEAR_REWARD_F_02,
                data.CLEAR_REWARD_F_03);
        buttonAction = func;
        GetIndex = _index;
        IsSelected = false;
    }
    ...
    public void SetReward(LevelRewardRawData data, UnityAction<int> func, int _index)
    {
        SetResource(data.LEVEL_REWARD_ICON, data.LEVEL_REWARD_NAME, data.LEVEL_REWARD_TEXT_ID,
            data.LEVEL_REWARD_01, data.LEVEL_REWARD_02);
        buttonAction = func;
        GetIndex = _index;
        IsSelected = false;
    }
